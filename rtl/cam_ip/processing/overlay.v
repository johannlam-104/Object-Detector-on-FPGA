...
module crossHairOverlay #(
    ...
    // indexing
    // -----------------------------
    wire [9:0] x_coor = i_tdata[18:9];
    wire [8:0] y_coor = i_tdata[8:0];
    
    wire is_red = i_tdata[19];
    
    wire [11:0] curr_rgb = i_tdata[31:20];
    wire [11:0] overlay = 12'h0F0; // green
    ...
    // crosshair predicate
    wire [9:0] dx = (x_coor > cen_x) ? (x_coor - cen_x) : (cen_x - x_coor);
    wire [8:0] dy = (y_coor > cen_y) ? (y_coor - cen_y) : (cen_y - y_coor);
    
    wire draw_crosshair = has_red && ((dx <= CROSSHAIR_SIZE) || (dy <= CROSSHAIR_SIZE)); // crosshair shape

    wire [31:0] overlay_pixel = draw_crosshair ? {overlay, is_red, x_coor, y_coor} : i_tdata; // dark green crosshair
    ...
    always @* begin
        ...
        case (state)
            ...
            S_DRAW: begin
                ...
                if (i_end_frame) begin // 5 tranfers early to actual eof
    ...
    always @(posedge i_clk) begin
      if (!i_rstn) begin
          ...
          // capture centroid decision on publish (early)
          ...
          else if (out_fire && !in_fire) begin
